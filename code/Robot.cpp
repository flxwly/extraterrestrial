    ...
    for (auto collision : ultraSonicContactPosition()) {
        if (collision) {
            if (map1.getMapAtPos(collision) == MAP_EMPTY_TILE) {
                ROBOT_WARNING(collision)
                ROBOT_WARNING("Marking unknown obstacle")
...
std::array<PVector, 3> Robot::ultraSonicContactPosition() {
    ...
    if (ultraSonicSensors.l < 100) {
        array[0] = simPos + geometry::angle2Vector((compass + ULTRASONIC_SENSOR_ANGLE_OFFSET) * M_PI / 180) *
                            (ultraSonicSensors.l + ULTRASONIC_SENSOR_DIST_TO_CORE);
    }

    if (ultraSonicSensors.f < 100) {
        array[0] = simPos + geometry::angle2Vector(compass * M_PI / 180) *

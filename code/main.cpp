...
void UpdateLoop() {
    ...
    CHAR_INFO chiBuffer[robot->map0.Map_.size()];
    COORD coordBufSize;
    COORD coordBufCoord;
    BOOL fSuccess;

    hStdout = GetStdHandle(STD_OUTPUT_HANDLE);
    coordBufSize.X = robot->map0.width_;
    coordBufSize.Y = robot->map0.height_;

    coordBufCoord.X = 0;
    coordBufCoord.Y = 0;

    srctWriteRect.Left = srctWriteRect.Top = 0;
    srctWriteRect.Right = robot->map0.width_ - 1;
    srctWriteRect.Bottom = robot->map0.height_ - 1;
    ...
    while (RunUpdateLoop) {
        ...
        for (int i = 0; i < robot->map0.Map_.size(); ++i)
        {
                chiBuffer[i].Char.UnicodeChar = robot->map0.Map_[i];
